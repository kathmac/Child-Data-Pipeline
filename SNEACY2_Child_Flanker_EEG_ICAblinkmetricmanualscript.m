...
%% change m_path to relevant folder
%% MUST RUN SECTION BY SECTION IN THIS SCRIPT
eeglab;
storage= '/Volumes/Data/ZShared/SNEACY/3_SNEACY2_HIIT_Child/SNEACY2_HIIT_EEG/Flanker_DL_EEG/Flanker_DL_EEG_Rest/Pre/ICA_BlinkMetricFail_ManualRemovalFiles/';  %where the raw problem files are stored
m_path= '/Volumes/Data/ZShared/SNEACY/3_SNEACY2_HIIT_Child/SNEACY2_HIIT_EEG/Flanker_DL_EEG/Flanker_DL_EEG_Rest/Pre/';  %where we aggregate with the other data
...
EEG = loadcurry([storage, Subject_list_EEG{subnum}], 'CurryLocations', 'False');
...
cd([m_path 'Set/'])
Subject_list_struct = dir('*FDL*.set'); % grab set files
...
% Assign Bins
 EEG  = pop_binlister( EEG , 'BDF', [m_path 'FL_DL_BinLister.txt'], 'ExportEL', ...
...
ERP = pop_binoperator( ERP, {'b5 = (b1+b2)/2 label Congruent Correct',...
    'b6 = (b3+b4)/2 label Incongruent Correct'}); 
...
ERP = pop_ploterps( ERP, [5 6],1:EEG.nbchan , 'Axsize', [ 0.05 0.08], 'BinNum', 'on', 'Blc', 'no', 'Box', ...
...
%% Manual inspection of components, if icablinkmetrics error - from Nicole - SKIP NEXT ICA SECTION & EYE BLINK REMOVAL 
    % Load BeforeICA .set manually in EEGlab
    % Tools --> reject data ICA -->reject component by map --> 1:20 components
    % --> look at noise, enter channels in section below to remove
    
    % Load channels to inspect
EEG = eeg_checkset( EEG );
pop_selectcomps(EEG, [1:20]);
 
% Change channels to reject
   % Manually identify and remove components [1 2] here (and note in text file)
    EEG = eeg_checkset( EEG );
 ...
 % Save before removing eye blinks
    ...
    % Load in .set file manually (File > Load existing dataset > select "_Before ICA file"
    % Reject components (Tools > Reject using ICA > Reject by Map) 
    % Manually identify and remove components [specify in brackets below] 
        %NOTE THE REMOVED COMPONENTS IN TEXT FILE 
        EEG = pop_subcomp( EEG, [1 2 3 4], 0);
        %after components are removed, inspect. Frontal hotspots should be removed 
...
% Save after manually removing eye blinks
    EEG = pop_saveset( EEG, 'filename', [subject '_AfterICA_aftereyeblinkremoved.set'],'filepath', [m_path 'ICA/Sets/']);
...
% % Save before removing eye blinks
% EEG = pop_saveset( EEG, 'filename', [subject '_AfterICA_beforeeyeblinkremoved'],'filepath', [m_path 'ICA/Sets/']);
% 
...
EEG = pop_loadset('filename',[subject '_AfterICA_aftereyeblinkremoved.set'],'filepath',[m_path 'ICA/Sets/']);
...
ERP = pop_ploterps( ERP, [5 6],1:EEG.nbchan , 'Axsize', [ 0.05 0.08], 'BinNum', 'on', 'Blc', 'no', 'Box', ...
    ...
    [ 103.714 29.6429 106.857 31.9286], 'Style', 'Classic', 'Tag', 'ERP_figure', 'Transparency',0, ...
    'xscale', [ -200.0 999.0 -200:200:800 ], 'YDir', 'reverse', 'yscale', ...
    [ -30.0 30.0   -30:7.5:-15 -7.6:7.6:7.6 15:7.5:30 ] ); %...ERP, [5 6]... = the bins you want to display
...
ERP = pop_loaderp( 'filename', [subject '_avg.erp'], 'filepath', [m_path 'Reductions/'] );                                                            
pop_export2text( ERP, [m_path 'Export_Excel/' subject '_erp_export.txt'], [5 6], 'electrodes', 'on', 'precision',4, 'time', 'on', 'timeunit',0.001 );
...
% filelist = dir('SA_HIIT_pr*_R_FDL_avg.erp');
...
% ERP = pop_savemyerp(ERP, 'erpname', 'SNEACY2_Child_pr_Flanker_R_', 'filename', 'SNEACY2_Child_pr_Flanker_R_.erp', 'filepath', [m_path 'ERPsets/'], 'Warning', 'off');   
% pop_export2text( ERP, [m_path 'Export_Excel/GrandAVG/','SNEACY2_Child_pr_Flanker_R_','_erp_export.txt'], [5 6], 'electrodes', 'on', 'precision',4, 'time', 'on', 'timeunit',0.001 );
% % % 
% % disp(',');
% % disp('--------------------------------');
