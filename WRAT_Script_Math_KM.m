...
FileName_Data = 'SNEACYChild-WRATMathInMATLABfrie_DATA_2021-03-25_1531.csv';
% SaveOutput_RM = 'SNEACY_Child_WRAT_Math_RM.xlsx';%copy and paste
% output...something wrong when exporting to excel. duplicates IDs
% SaveOutput_Long= 'SNEACY_Child_WRAT_Math_Long.xlsx';
%% Import the standard scores
cd(FilePath);
StandardScores_Blue = readtable(FileName_Blue, 'Sheet', 'WRAT4_MathComputation');
...
cd(FilePath);
StandardScores_Green = readtable(FileName_Green, 'Sheet', 'WRAT4_MathComputation');
...
%% Import the data
cd(FilePath);
MathComputation = readtable(FileName_Data);

%% Organizing Data
%making the table with numbers instead of strings
MathComputation.wrat_b_math_score=str2double(MathComputation.wrat_b_math_score);
%pulling age from MathComputation 
Age = MathComputation(MathComputation.redcap_event_name==string('baseline_parent_su_arm_1'),:);
%Remove unneeded columns
Age = removevars(Age,{'redcap_event_name','wrat_g_int','wrat_g_math_score','wrat_b_int','wrat_b_math_score'});
%pulling intervention and scores from MathComputation
Int_Score=MathComputation(MathComputation.redcap_event_name==string('wrat_arm_1'),:);
%remove unneeded columns
...
%% Delete row with missing age info (SA317, sA140)- FIX AGE AND DELETE 
Combined_age_int_score(134,:) = [];
...
for P = 1:DataTableRows
    if Combined_age_int_score.wrat_g_math_score(P) > 0
        temp = char(Combined_age_int_score.CatAge(P));
        StandScore = array2table(StandardScores_Green.(temp)(Combined_age_int_score.wrat_g_math_score(P))) ;
       Combined_age_int_score(P,'wrat_g_math_standard') = StandScore;
    elseif ismissing(Combined_age_int_score.wrat_g_math_score(P))
...
for P = 1:DataTableRows
    if Combined_age_int_score.wrat_b_math_score(P) > 0
        temp = char(Combined_age_int_score.CatAge(P));
        StandScore = array2table(StandardScores_Blue.(temp)(Combined_age_int_score.wrat_b_math_score(P))) ;
        Combined_age_int_score(P,'wrat_b_math_standard') = StandScore;
    elseif ismissing(Combined_age_int_score.wrat_g_math_score(P))
        StandScore = array2table(NaN);
        Combined_age_int_score(P,'wrat_b_math_standard') = StandScore;
    end
...
SNEACY_WRAT_G = Combined_age_int_score(:,{'Subject','Age','wrat_g_int','wrat_g_math_score','wrat_g_math_standard'}); %pull out green scores
SNEACY_WRAT_G.Properties.VariableNames{3} = 'WRAT_Int'; % change to neutral name
SNEACY_WRAT_G.Properties.VariableNames{4} = 'WRAT_Math_RawScore'; % change to neutral name
SNEACY_WRAT_G.Properties.VariableNames{5} = 'WRAT_Math_StandScore'; % change to neutral name
...
SNEACY_WRAT_B = Combined_age_int_score(:,{'Subject','Age','wrat_b_int','wrat_b_math_score','wrat_b_math_standard'}); %pull out blue scores
SNEACY_WRAT_B.Properties.VariableNames{3} = 'WRAT_Int'; % change to neutral name
SNEACY_WRAT_B.Properties.VariableNames{4} = 'WRAT_Math_RawScore'; % change to neutral name
SNEACY_WRAT_B.Properties.VariableNames{5} = 'WRAT_Math_StandScore'; % change to neutral name
...
Variable_Names = {'WRAT_Math_RawScore_T'; 'WRAT_Math_RawScore_R'; 'WRAT_Math_RawScore_E'};  %create variable names for each variable
SNEACY_WRAT_RM_Raw = unstack(SNEACY_WRAT_Long, 'WRAT_Math_RawScore', 'WRAT_Int', 'GroupingVariables', 'Subject', 'NewDataVariableNames', Variable_Names);

% Create StandScore RM variables
Variable_Names = {'WRAT_Math_StandScore_T'; 'WRAT_Math_StandScore_R'; 'WRAT_Math_StandScore_E'};  %create variable names for each variable
SNEACY_WRAT_RM_Standard = unstack(SNEACY_WRAT_Long, 'WRAT_Math_StandScore', 'WRAT_Int', 'GroupingVariables', 'Subject', 'NewDataVariableNames', Variable_Names);
...
% SNEACY_WRAT_RM_bycolor = Combined_age_int_score(:,{'Subject','Age','wrat_g_int','wrat_g_math_standard','wrat_b_int','wrat_b_math_standard'});
% SNEACY_WRAT_RM_bycolor(end,:) = []; %delete sA140, which is labeled incorrectly and has no data and fucks shit up
% 
% Variable_Names = {'G_WRAT_Math_StandScore_T'; 'G_WRAT_Math_StandScore_R'; 'G_WRAT_Math_StandScore_E'};  %create variable names for each variable
% SNEACY_WRAT_RM_G = unstack(SNEACY_WRAT_RM_bycolor, 'wrat_g_math_standard', 'wrat_g_int', 'GroupingVariables', 'Subject', 'NewDataVariableNames', Variable_Names);
% 
% Variable_Names = {'B_WRAT_Math_StandScore_T'; 'B_WRAT_Math_StandScore_R'; 'B_WRAT_Math_StandScore_E'};  %create variable names for each variable
% SNEACY_WRAT_RM_B = unstack(SNEACY_WRAT_RM_bycolor, 'wrat_b_math_standard', 'wrat_b_int', 'GroupingVariables', 'Subject', 'NewDataVariableNames', Variable_Names);
% 
% SNEACY_WRAT_RM_bycolor = removevars(SNEACY_WRAT_RM_bycolor, {'wrat_g_int','wrat_g_math_standard','wrat_b_int','wrat_b_math_standard'});%delete old variables 
% Age.Properties.VariableNames{1} = 'Subject';
% Age.Properties.VariableNames{2} = 'Age';
% 
...
%% Saving Excel File
cd(FilePath)
save SNEACY_WRAT_Math %save output as Matlab variable
